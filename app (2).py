...
# ================== STRICT RED BUTTON STYLING FOR ALL USERS/ADMINS ==================
def _determine_style(text: str) -> str:
    """Force cancel, back, contact owner, owner panel, admin panel, stop, and input action buttons to red."""
    low = text.lower()
    if any(k in low for k in ("cancel", "stop", "remove", "delete", "disable", "ban", "deduct", "reset", "danger", "contact owner", "back", "owner panel", "admin panel", "number", "message", "speed", "count")):
        return "danger"
    if any(k in low for k in ("start", "send", "add", "generate", "enable", "buy", "refresh", "success", "yes")):
        return "success"
    return "danger"
    ...
    for row in rows:
        ...
        for item in row:
            if isinstance(item, tuple) and len(item) == 2:
                text, callback = item
                style = _determine_style(text)
                new_row.append(btn(text, callback, style=style))
            else:
                new_row.append(item)
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn_url("👑 CONTACT OWNER", SUPER_ADMIN_LINK, EMOJI_CROWN, "👑", style="danger")],
    ])
...
def videos_menu_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    rows.append([btn("ʙᴀᴄᴋ", "owner:home", EMOJI_GEAR, "🔙", style="danger")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def unban_menu_kb(d: dict, prefix: str) -> InlineKeyboardMarkup:
    ...
    rows.append([btn("ʙᴀᴄᴋ", f"{prefix}:home", EMOJI_GEAR, "🔙", style="danger")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
# ================== ROBUST 10-25 MINUTE STEADY ATTACK LOOP WITH 3X MULTIPLIER ==================
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    speed_label_display = "🚀 FAST (2X)" if count >= 2000 else "🚀 FAST" if speed == SPEED_FAST else "⚡ MEDIUM" if speed == SPEED_MEDIUM else "🐢 SLOW"
    ...
    # Pacing strategy for 10-25 min duration when needed
    target_min_duration = 600.0  # 10 minutes minimum target if count is large
    if actual_count > 100:
        calculated_delay = target_min_duration / actual_count
        speed = max(speed, calculated_delay)

    async def do_send():
        nonlocal sent_ok, sent_fail, msgs_left, last_update_time, failed_attempts, burst_counter
        sem = asyncio.Semaphore(_MAX_CONCURRENT_SENDS)

        try:
            while msgs_left > 0 and not session.cancelled:
                tasks = []
                for _ in range(min(_MAX_CONCURRENT_SENDS, msgs_left)):
                    ...
                    sim_slot = random.choice(sims).get("simSlotIndex", 0) if sims else 0

                    async def send_one(fb_url, dev_id, sim_slot, number, message):
                        async with sem:
                            try:
                    ...
                    tasks.append(send_one(fb_url, dev_id, sim_slot, number, message))
                ...
                for ok in results:
                    ...
                    async with session.lock:
                        ...
                        now = time.time()
                        ui_sent = min(count, int(sent_ok / 3))
                        ui_fail = min(count - ui_sent, int(sent_fail / 3))
                        ...
                        if (now - last_update_time >= _PROGRESS_UPDATE_INTERVAL or
                            ...
                            try:
                                await progress_msg.edit_text(
                                    progress_text(ui_sent, ui_fail, count, current_credits_live, speed_label_display),
                    ...
                    if burst_counter >= BURST_SIZE and not session.cancelled:
                        await asyncio.sleep(REST_TIME)
                        burst_counter = 0

                    await asyncio.sleep(speed)

                if sent_ok == 0 and msgs_left > 0:
                    failed_attempts += 1
                    if failed_attempts > MAX_FAILED_ATTEMPTS:
                        log.warning(f"User {uid}: Reached max consecutive failures, continuing robust session")
                        failed_attempts = 0
                    await asyncio.sleep(1.0)

                # Prevent any timeout or connection break
    ...
    final_ui_sent = int(sent_ok / 3)
    final_ui_fail = int(sent_fail / 3)
    ...
    log_activity(d_log, "sms_blast", uid,
        f"Sent: {sent_ok}, Failed: {sent_fail}, Total: {actual_count}, Duration: {fmt_duration(duration)}, Stopped: {was_cancelled}")
    save(d_log)
    ...
    chan_log = (
        ...
        f"✅ <b>Sent:</b> <b>{final_ui_sent}</b>\n"
        f"❌ <b>Failed:</b> <b>{final_ui_fail}</b>\n"
    ...
    if final_ui_fail == 0 and final_ui_sent > 0:
        icon = em(EMOJI_CHECK, "✅")
    elif final_ui_sent > 0:
    ...
    if is_regular_user:
        ...
        credit_text = f"\n{em(EMOJI_MONEY, '💰')} Credits Used: <b>{final_ui_sent}</b>\n{em(EMOJI_MONEY, '💳')} Remaining: <b>{remaining}</b>"
    ...
    if is_owner(uid, load()):
        back_btn = [btn("ᴏᴡɴᴇʀ ᴘᴀɴᴇʟ", "owner:home", EMOJI_GEAR, "🔙", style="danger")]
    elif is_admin(uid, load()):
        back_btn = [btn("ᴀᴅᴍɪɴ ᴘᴀɴᴇʟ", "admin:home", EMOJI_GEAR, "🔙", style="danger")]
    else:
        back_btn = [btn("sᴇɴᴅ ᴀɴᴏᴛʜᴇʀ", "user:send", EMOJI_ROCKET, "📤", style="success"), btn("ʜᴏᴍᴇ", "user:home", EMOJI_STAR, "🏠", style="danger")]
    ...
    try:
        await progress_msg.edit_text(
            ...
            f"{em(EMOJI_CHECK, '✅')} Sent: <b>{final_ui_sent}</b>\n"
            f"{em(EMOJI_CROSS, '❌')} Failed: <b>{final_ui_fail}</b>\n"
...
async def user_stop_send(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        async with session.lock:
            current_sent = int(session.sent / 3)
            current_failed = int(session.failed / 3)
        await cq.message.edit_text(
...
async def owner_protected_remove_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("ʙᴀᴄᴋ", "owner:protected_list", EMOJI_GEAR, "🔙", style="danger")])
...
async def owner_fj_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        ...
        [btn("ʙᴀᴄᴋ", "owner:home", EMOJI_GEAR, "🔙", style="danger")]
    ]
...
async def owner_fj_remove_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("ʙᴀᴄᴋ", "owner:fj:menu", EMOJI_GEAR, "🔙", style="danger")])
...
async def owner_pricing_remove(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("ʙᴀᴄᴋ", "owner:pricing:menu", EMOJI_GEAR, "🔙", style="danger")])
...
async def owner_redeem_del_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("ʙᴀᴄᴋ", "owner:redeem:menu", EMOJI_GEAR, "🔙", style="danger")])
...
async def user_pricing(cq: CallbackQuery, state: FSMContext):
    ...
    rows.append([btn("ʙᴀᴄᴋ", "user:home", EMOJI_GEAR, "🔙", style="danger")])
