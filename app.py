...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    async def do_send():
        ...
        try:
            MAX_CYCLES = 3        # Poora loop sirf 3 baar chalega
            PER_DEVICE = 6        # Har device se max 6 SMS per cycle
                ...
                for device in devices:
                    ...
                    async with session.lock:
                        if session.cancelled:
                            log.info(f"User {uid} stopped at cycle {cycle+1}, sent={sent_ok}")
                            break
                    ...
                    device_quota = min(PER_DEVICE, msgs_left)
                    device_sent  = 0
