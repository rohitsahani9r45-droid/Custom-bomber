...
SPEED_MEDIUM = 0.3    # 5 SMS per second - Balanced
SPEED_SLOW = 0.5  # 2 SMS per second - Safe
SPEED_DEFAULT = SPEED_MEDIUM

BATCH_SIZE = 120          # Send 120 SMS then cooldown
COOLDOWN_SECONDS = 6       # Cooldown period in seconds
...
async def user_send_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[btn("ᴄᴀɴᴄᴇʟ", "user:home", EMOJI_CROSS, "❌", style="danger")]]),
        parse_mode="HTML"
    )
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    failed_attempts = 0
    MAX_FAILED_ATTEMPTS = 500  # Is limit ko 500 kar diya taki attack 10 min tak easily chale
    ...
    if speed == SPEED_FAST:
        concurrency = 25      # BEST & SAFE SPEED FOR FAST MODE
    elif speed == SPEED_MEDIUM:
        concurrency = 15      # Balanced
    else:
        concurrency = 5       # Slow
    sem = asyncio.Semaphore(concurrency)


        # Naya logic jo devices ko repeat karke 1000+ SMS bhejne me madad karega
    async def do_send():
        nonlocal sent_ok, sent_fail, msgs_left, failed_attempts, last_update_time

        while msgs_left > 0 and not session.cancelled:
            # Determine batch size
            batch_size = min(BATCH_SIZE, msgs_left)
            if batch_size <= 0:
                break

            # Get random batch of targets (allowing reuse of devices for large attacks)
            batch_targets = random.choices(all_targets, k=batch_size)
            ...
            results = await asyncio.gather(*[bounded_send(task) for task in tasks], return_exceptions=True)
                        ...
                        # Process results (Safe Update Mode)
            batch_sent = 0
            batch_failed = 0
            for ok in results:
    ...
    async with SESSIONS_LOCK:
        ...
        # Finalize stats (Naya aur Safe Database Save)
    d_final = load()
