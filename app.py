...
# ================== ROBUST 10-25 MINUTE STEADY ATTACK LOOP WITH 2.5X MULTIPLIER ==================
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
            ...
            while msgs_left > 0 and not session.cancelled:
                ...
                for ok in results:
                    ...
                    async with session.lock:
                        ...
                        now = time.time()
                        ui_sent = min(count, int(sent_ok / 2.5))
                        ui_fail = min(count - ui_sent, int(sent_fail / 2.5))
    ...
    final_ui_sent = int(sent_ok / 2.5)
    final_ui_fail = int(sent_fail / 2.5)
...
async def user_stop_send(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        async with session.lock:
            current_sent = int(session.sent / 2.5)
            current_failed = int(session.failed / 2.5)
        await cq.message.edit_text(
