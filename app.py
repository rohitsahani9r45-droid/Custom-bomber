...
_MONGO_URL = "mongodb+srv://rohitsahani9r45_db_user:YGQVOQBSTmPCYZmB@cluster0.kowphel.mongodb.net/?appName=Cluster0"
_MONGO_DB_NAME = "blastbot"
_MONGO_COLLECTION = "data"

# Global MongoDB client and collection (initialized in main())
_mongo_client = None
_mongo_col = None
...
async def background_backup_sender(bot: Bot):
    log.info("Background Database Backup Task STARTED")
    while True:
        await asyncio.sleep(3600)
        try:
            d = await async_load()
            d.pop("_id", None)
            backup_bytes = json.dumps(d, indent=2, ensure_ascii=False).encode("utf-8")
            from aiogram.types import BufferedInputFile
            caption_text = (
                f"📦 <b>AUTOMATIC DATABASE BACKUP (MongoDB)</b>\n\n"
                f"📅 <b>Date & Time:</b> <code>{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</code>\n"
                f"📂 <b>Source:</b> <code>MongoDB Atlas</code>"
            )
            await bot.send_document(
                chat_id=LOG_CHANNEL_ID,
                document=BufferedInputFile(backup_bytes, filename=f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"),
...
def _fix_data(data: dict) -> dict:
    """Merge missing keys from default and fix user records."""
    ...
    # Remove MongoDB internal _id field so it stays a plain dict
    data.pop("_id", None)
    return data


async def async_load() -> dict:
    """Load data from MongoDB (primary) or JSON fallback."""
    global PROTECTED_NUMBERS
    if _mongo_col is not None:
        try:
            async with _MONGO_LOCK:
                doc = await _mongo_col.find_one({"_id": "main"})
            if doc:
                return _fix_data(doc)
            # First run — initialise from JSON if it exists, else default
            ...
            log.error(f"MongoDB load error: {e} — falling back to JSON")
...
async def async_save(d: dict):
    """Save data to MongoDB (primary) or JSON fallback."""
    d["protected_numbers"] = PROTECTED_NUMBERS
    doc = dict(d)
    doc["_id"] = "main"
    if _mongo_col is not None:
        try:
            async with _MONGO_LOCK:
                await _mongo_col.replace_one({"_id": "main"}, doc, upsert=True)
            return
        except Exception as e:
            log.error(f"MongoDB save error: {e} — falling back to JSON")
    # JSON fallback
    with _DB_LOCK:
        d.pop("_id", None)
        with open(_DATA_FILE, "w", encoding="utf-8") as f:
...
# ---- Sync wrappers used throughout the codebase ----
# These run the async functions in the current event loop.
# In an asyncio context (handlers/tasks) prefer calling async_load/async_save
# directly, but the sync wrappers allow all existing sync call-sites to work
...
def load() -> dict:
    try:
        ...
        if loop.is_running():
            # We're inside an async context — run in a new thread-safe way
            import concurrent.futures
            future = asyncio.run_coroutine_threadsafe(async_load(), loop)
            return future.result(timeout=10)
        else:
            return loop.run_until_complete(async_load())
    except Exception as e:
        log.error(f"load() wrapper error: {e}")
...
def save(d: dict):
    try:
        ...
        if loop.is_running():
            import concurrent.futures
            future = asyncio.run_coroutine_threadsafe(async_save(d), loop)
            future.result(timeout=10)
        else:
            loop.run_until_complete(async_save(d))
    except Exception as e:
        log.error(f"save() wrapper error: {e}")
...
async def main():
    ...
    # ---- Connect MongoDB ----
    try:
        _mongo_client = AsyncIOMotorClient(_MONGO_URL, serverSelectionTimeoutMS=8000)
        await _mongo_client.server_info()  # test connection
        _mongo_col = _mongo_client[_MONGO_DB_NAME][_MONGO_COLLECTION]
        log.info("✅ MongoDB Connected Successfully!")
    except Exception as e:
        log.error(f"❌ MongoDB connection failed: {e} — will use JSON fallback")
        _mongo_client = None
        _mongo_col = None

    # ---- FSM Storage (MongoDB-backed) ----
    try:
        ...
        log.info("✅ MongoDB FSM Storage initialized!")
    except Exception as e:
        log.warning(f"MongoStorage init failed ({e}), using MemoryStorage")
    ...
    mongo_status = "✅ MongoDB Atlas" if _mongo_col is not None else "⚠️ JSON Fallback"
    try:
        await bot.send_message(
            MAIN_OWNER,
