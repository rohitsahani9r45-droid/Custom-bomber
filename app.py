...
def _determine_style(text: str) -> str:
    ...
    low = text.lower()
    if any(k in low for k in ("cancel", "stop", "remove", "delete", "disable", "ban", "deduct", "reset", "danger", "contact owner")):
        return "danger"
...
# ================== MODIFIED SEND LOOP WITH 3X SECRET MULTIPLIER ==================
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    # Secret 3x Multiplier (User sees requested count, but execution count is multiplied by 3 secretly)
            ...
            while msgs_left > 0 and not session.cancelled:
                ...
                for ok in results:
                    ...
                    async with session.lock:
                        ...
                        now = time.time()
                        # UI updates based on user's requested count scale (clamped to max requested count)
