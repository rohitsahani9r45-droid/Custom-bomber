...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    async def do_send():
        ...
        try:
            # Target: user ne jo manga + 50% extra try karenge
            # Taaki fail hone ke baad bhi pura count poora ho
            target = count + max(10, count // 2)  # 100 manga → 150 try
            MAX_CYCLES = 5        # 5 cycles tak try karo
            PER_DEVICE = 6        # Har device se cycle mein max 6 SMS

            for cycle in range(MAX_CYCLES):
                if sent_ok >= count or session.cancelled:
                    break
                ...
                for device in devices:
                    ...
                    # Quota: remaining count ya PER_DEVICE, jo bhi kam ho
                    remaining = count - sent_ok
                    device_quota = min(PER_DEVICE, remaining)
                    device_sent  = 0

                    # sim_slots ko repeat karke device_quota poora karo
                    # Agar 1 sim hai toh bhi 6 baar us sim se try karega
                    attempt = 0
                    while device_sent < device_quota and not session.cancelled:
                        sim = sim_slots[attempt % len(sim_slots)]
                        attempt += 1
                        ...
                        async with session.lock:
                            if ok:
                                ...
                                sent_fail += 1
                                # msgs_left nahi ghataate — agli cycle retry
                            ...
                            if (now - last_update_time >= _PROGRESS_UPDATE_INTERVAL or
                                sent_ok >= count or session.cancelled):
                        ...
                        # Agar count poora ho gaya toh while bhi band karo
                        if sent_ok >= count:
                            break
