...
def progress_text(sent: int, failed: int, total: int, credits: int = None, speed_label: str = "⚡ MEDIUM", cooldown: bool = False) -> str:
    ...
    if cooldown:
        lines.append(f"\n{em(EMOJI_WARNING, '⏱')} <b>Cooldown...</b> {COOLDOWN_SECONDS}s")
...
# ===== MODIFIED: Shorter timeout and fewer retries for fast failover =====
async def fb_put(base_url: str, path: str, payload: dict) -> bool:
    url = base_url.rstrip("/") + path
    # Only one attempt, timeout 1.5 seconds
    try:
        async with aiohttp.ClientSession() as s:
            async with s.put(url, json=payload, timeout=aiohttp.ClientTimeout(total=1.5)) as r:
                if 200 <= r.status < 300:
                    return True
    except Exception as e:
        log.warning(f"fb_put failed: {e}")
    return False
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("🔴 ᴄᴏɴᴛᴀᴄᴛ ʀᴏʜɪᴛ", "user:contact_rohit", EMOJI_CROSS, "🔴", style="danger")],  # NEW: red contact button
    ])
...
def admin_kb(d: dict) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("🔴 ᴄᴏɴᴛᴀᴄᴛ ʀᴏʜɪᴛ", "user:contact_rohit", EMOJI_CROSS, "🔴", style="danger")],  # NEW
    ])
...
# ===== MODIFIED: run_sms_blast_with_progress with batch cooldown =====
    ...
    async def do_send():
        ...
        try:
            while msgs_left > 0 and not session.cancelled:
                batch_sent = 0
                # Shuffle devices for load balancing each batch
                ...
                for device in device_queue:
                    ...
                    for sim in sim_slots:
                        ...
                        # Attempt to send with fast failover (timeout already in fb_put)
                        try:
                        ...
                        async with session.lock:
                            if ok:
                                ...
                                # Update global stats for non-regular users later
                            else:
                                sent_fail += 1
                                failed_attempts += 1
                        ...
                        # Sleep only on success, skip on failure for fast failover
                        if ok:
                            await asyncio.sleep(speed)
                        else:
                            # No sleep on failure to move to next device immediately
                            pass

                # After sending a batch, if we still have messages left and we reached batch size, cooldown
                if msgs_left > 0 and not session.cancelled and batch_sent >= BATCH_SIZE:
                    # Cooldown: wait and update progress with cooldown message
                    ...
                    while time.time() - cooldown_start < COOLDOWN_SECONDS and not session.cancelled:
                        ...
                        if remaining > 0:
                            # Update progress with cooldown indicator
                            current_credits_live = get_user_credits(uid, load()) if is_regular_user else None
                            try:
                                await progress_msg.edit_text(
                    ...
                    # After cooldown, continue loop for next batch

                # If batch_sent == 0 and msgs_left > 0, we are stuck; increment failure counter
