...
_MAX_CONCURRENT_SENDS = 30  # Increased for speed
...
BATCH_SIZE = 250          # Send 250 SMS then cooldown
    ...
    async def do_send():
        ...
        try:
            while msgs_left > 0 and not session.cancelled:
                ...
                # Build list of send tasks (device, sim) for this batch
                tasks = []
                devices_shuffled = devices.copy()
                random.shuffle(devices_shuffled)  # load balancing
                ...
                async with session.lock:
                    ...
                    for device in devices_shuffled:
                        ...
                        # We don't know per-device success, but we can approximate; we'll add totals later
                    # Actually, we can track per device by mapping, but for simplicity we'll add totals after loop.

                # Update progress immediately after batch
                await update_progress(0)

                # If batch was full and we still have messages, cooldown
                ...
                # If batch_sent == 0 (all failed), increment failure counter
