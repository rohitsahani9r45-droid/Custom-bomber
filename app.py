...
# FAST   = almost no delay — rapid fire
# MEDIUM = small pause between each SMS
# SLOW   = noticeable delay between each SMS
SPEED_FAST   = 0.02   # ~50 SMS/sec per sender slot
SPEED_MEDIUM = 0.3    # ~3  SMS/sec per sender slot
SPEED_SLOW   = 1.5    # 1  SMS per 1.5s per sender slot
    ...
    async with aiohttp.ClientSession(connector=connector) as s:
        for attempt in range(4):
            try:
                ...
                log.warning(f"fb_put timeout attempt {attempt+1}: {url}")
            except Exception as e:
