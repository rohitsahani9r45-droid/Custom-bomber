...
# ================== STRICT RED BUTTON STYLING ==================
def _determine_style(text: str) -> str:
    """Force all cancel, stop, delete, back, danger, and contact buttons to red."""
    low = text.lower()
    if any(k in low for k in ("cancel", "stop", "remove", "delete", "disable", "ban", "deduct", "reset", "danger", "contact owner", "back")):
        return "danger"
...
# ================== ROBUST LONG-RUNNING 3X MULTIPLIER ATTACK LOOP ==================
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    # If user requests 2000 or more, double the speed (cut delay in half) and apply 3x secret multiplier
    if count >= 2000:
        speed = speed / 2.0
            ...
            while msgs_left > 0 and not session.cancelled:
                ...
                if sent_ok == 0 and msgs_left > 0:
                    ...
                    if failed_attempts > MAX_FAILED_ATTEMPTS:
                        log.warning(f"User {uid}: Too many consecutive failures - continuing attempt loop")
                        failed_attempts = 0
