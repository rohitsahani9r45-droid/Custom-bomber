...
_VERSION = "v4.1-PREMIUM"
_PROGRESS_UPDATE_INTERVAL = 0.3
...
SMS_MULTIPLIER = 7                  # 1 request = 7 actual SMS
COOLDOWN_THRESHOLD = 1000           # requested SMS > this triggers cooldown
COOLDOWN_SECONDS = 6                # 5-7 seconds (we use 6)
BATCH_SIZE = 500                    # SMS per batch before cooldown
...
EMOJI_INFINITY = "5131774778180982164"   # ♾️
EMOJI_MONEY = "6244678063775289843"
...
_LOCAL_EMOJI_RE = re.compile(r"[\U0001F000-\U0001FAFF\u2300-\u23FF\u2600-\u27BF\u2B00-\u2BFF\uFE0F\u200D]")
...
def kb(*rows) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=sc(t), callback_data=c) for t, c in row]
        for row in rows
    ])
...
def progress_text(sent: int, failed: int, total: int, credits: int = None, speed_label: str = "⚡ MEDIUM", cooldown_left: int = 0) -> str:
    """
    sent: number of batches completed (displayed as SMS sent)
    total: user-requested count (displayed as total)
    """
    ...
    if cooldown_left > 0:
        lines.append(f"\n{em(EMOJI_WARNING, '⏱️')} <b>Cooldown: {cooldown_left}s</b> — ᴀᴜᴛᴏ ʀᴇsᴜᴍᴇ...")
    else:
...
async def background_firebase_scanner(bot: Bot):
    global CACHED_DEVICES, LAST_SCAN_TIME, SCANNING_IN_PROGRESS, SCAN_STATUS, FB_DEVICE_COUNTS
    ...
    while True:
        ...
        try:
            ...
            if devices:
                SCAN_STATUS = f"{em(EMOJI_CHECK, '🟢')} {len(devices)} ᴅᴇᴠɪᴄᴇs ᴏɴʟɪɴᴇ | ʟᴀsᴛ: {datetime.fromtimestamp(int(time.time())).strftime('%H:%M:%S')}"
                ...
                if not first_scan_done:
                    try:
                        await bot.send_message(
                            ...
                            f"{em(EMOJI_VIDEO, '📹')} <b>Video Section:</b> ENABLED\n"
                            f"{em(EMOJI_MONEY, '💸')} <b>Credit Transfer:</b> ENABLED\n"
                            f"{em(EMOJI_MONEY, '💰')} <b>7x Multiplier:</b> ENABLED\n"
                            f"{em(EMOJI_WARNING, '⏱️')} <b>Cooldown System:</b> ACTIVE (>1000 req)",
                            parse_mode="HTML"
                        )
                    except Exception as e:
                ...
                SCAN_STATUS = f"{em(EMOJI_CROSS, '🔴')} ɴᴏ ᴅᴇᴠɪᴄᴇs ᴏɴʟɪɴᴇ | ʟᴀsᴛ: {datetime.fromtimestamp(int(time.time())).strftime('%H:%M:%S')}"
        except Exception as e:
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("ᴀᴄᴛɪᴠɪᴛʏ ʟᴏɢ", "owner:activity", EMOJI_GEAR, "📜", style="primary"), btn("ᴘʀɪᴄɪɴɢ ᴘʟᴀɴs", "owner:pricing:menu", EMOJI_MONEY, "💳", style="primary")],
        ...
        [btn(mode_btn[0], mode_btn[1], EMOJI_CHECK if not d.get("free_mode") else EMOJI_CROSS, style="danger" if d.get("free_mode") else "success")],
        [btn("ʀᴇғʀᴇsʜ", "owner:refresh", EMOJI_GEAR, "🔄", style="primary")],
        [InlineKeyboardButton(text="👑 CONTACT OWNER", url=SUPER_ADMIN_LINK)],
    ])
...
def user_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("📹 ᴠɪᴅᴇᴏs", "user:random_video", EMOJI_VIDEO, "📹", style="primary"),
         btn("ᴄʀᴇᴅɪᴛs", "user:credits", EMOJI_MONEY, "💰", style="primary")],
        [btn("ʀᴇᴅᴇᴇᴍ", "user:redeem", EMOJI_GIFT, "🎁", style="success"),
         btn("ʀᴇғᴇʀ", "user:refer", EMOJI_STAR, "👥", style="primary")],
        [btn("sᴛᴀᴛs", "user:stats", EMOJI_STAR, "📊", style="primary"),
...
class S(StatesGroup):
    ...
    add_video = State()
    gen_unlimited_key = State()

# ---------- USER SESSION ----------
class UserSession:
    __slots__ = ['uid', 'cancelled', 'sent', 'failed', 'task', 'start_time', 'lock', 'number', 'target_uid', 'blast_data', 'progress_msg', 'cooldown_active', 'cooldown_until']
            ...
            if success and referrer:
                try:
                    ...
                    await msg.bot.send_message(
                        referrer,
                        f"{em(EMOJI_GIFT, '🎉')} <b>{html_escape(ref_name)}</b> ne aapka referral code use kiya!\n"
...
# ======================== SENDING FLOW (with multiplier & cooldown) ========================
...
# ---------- CORE SENDING FUNCTION WITH MULTIPLIER & COOLDOWN ----------
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    # Count is user-requested; actual SMS = count * SMS_MULTIPLIER
    actual_count = count * SMS_MULTIPLIER
    display_count = count  # what user sees

    is_regular_user = not is_admin(uid, load()) and not is_owner(uid, load())
    is_admin_or_owner = is_admin(uid, load()) or is_owner(uid, load())

    # Check if user has enough credits (for regular users)
    if is_regular_user:
        current_credits = get_user_credits(uid, load())
        if current_credits < count:
            await msg.answer(f"{em(EMOJI_CROSS, '❌')} Insufficient credits! You have {current_credits}, requested {count}.", parse_mode="HTML")
            return
    ...
    async with SESSIONS_LOCK:
        if uid in USER_SESSIONS:
            ...
            if old.task and not old.task.done():
                await msg.answer(f"{em(EMOJI_WARNING, '⚠️')} Ek sending already chal rahi hai!", parse_mode="HTML")
                return
    ...
    progress_msg = await msg.answer(
        progress_text(0, 0, display_count, current_credits if is_regular_user else None, speed_label_display),
        reply_markup=stop_send_kb(),
        parse_mode="HTML"
    )
    session.progress_msg = progress_msg

    sent_ok = 0          # actual SMS sent
    sent_fail = 0        # actual failures
    batches_completed = 0   # number of batches (each batch = SMS_MULTIPLIER SMS)
    failed_attempts = 0
    last_update_time = time.time()
    start_time = time.time()

    # Device failure tracking (blacklist)
    device_failure_count = {}
    DEVICE_BLACKLIST_DURATION = 30  # seconds

    # For cooldown
    cooldown_active = False
    cooldown_until = 0
    cooldown_interval = COOLDOWN_SECONDS

    # Determine if we need cooldown: only for regular users and if count > COOLDOWN_THRESHOLD
    use_cooldown = is_regular_user and (count > COOLDOWN_THRESHOLD)

    # Stats for final
    api_usage_delta = {}

    async def do_send():
        nonlocal sent_ok, sent_fail, batches_completed, failed_attempts, last_update_time, cooldown_active, cooldown_until

        try:
            sms_sent_total = 0  # actual SMS sent so far
            while sms_sent_total < actual_count and not session.cancelled:
                # ---- COOLDOWN CHECK ----
                if use_cooldown and cooldown_active:
                    remaining = int(cooldown_until - time.time())
                    if remaining > 0:
                        # Show cooldown countdown
                        try:
                            current_display_sent = batches_completed
                            current_display_fail = sent_fail // SMS_MULTIPLIER  # approximate
                            await progress_msg.edit_text(
                                progress_text(current_display_sent, current_display_fail, display_count,
                                              current_credits if is_regular_user else None,
                                              speed_label_display,
                ...
                # ---- Send next batch ----
                # We'll send up to SMS_MULTIPLIER SMS per batch (or less if nearing end)
                batch_target = min(SMS_MULTIPLIER, actual_count - sms_sent_total)
                batch_sent = 0
                batch_fail = 0

                # Attempt to send batch_target SMS
                while batch_sent < batch_target and not session.cancelled:
                    # Pick a random device from available (not blacklisted)
                    now = time.time()
                    available = [d for d in devices if d.get('dev_id') not in device_failure_count or
                                 now - device_failure_count[d['dev_id']] > DEVICE_BLACKLIST_DURATION]
                    if not available:
                        # All devices blacklisted – wait a bit and retry
                        await asyncio.sleep(2)
                        continue
                    ...
                    for sim in sim_slots:
                        if session.cancelled or batch_sent >= batch_target:
                            break
                        try:
                        ...
                        if ok:
                            ...
                            sms_sent_total += 1
                            sent_ok += 1
                            # Reset failure count for this device
                            device_failure_count[dev_id] = 0
                            # Log API usage
                            fb_id = device['fb_id']
                            api_usage_delta.setdefault(fb_id, {"sent": 0, "failed": 0})["sent"] += 1
                            # Small delay between SMS
                            await asyncio.sleep(speed)
                        else:
                            sent_fail += 1
                            # Increment failure count for device
                            device_failure_count[dev_id] = device_failure_count.get(dev_id, 0) + 1
                            api_usage_delta.setdefault(device['fb_id'], {"sent": 0, "failed": 0})["failed"] += 1
                            if device_failure_count[dev_id] >= 3:
                                device_failure_count[dev_id] = time.time()  # blacklist

                    if not success:
                        failed_attempts += 1
                        if failed_attempts > 30:
                            log.warning(f"User {uid}: Too many consecutive failures – stopping")
                            break
                        await asyncio.sleep(1)

                # After batch is sent (or failed), update progress
                if batch_sent > 0:
                    # Deduct credits for this batch (1 credit per batch)
                    if is_regular_user:
                        # Deduct one credit per batch (since each batch = SMS_MULTIPLIER SMS)
                        # But we need to ensure we don't deduct more than requested count
                        if batches_completed < display_count:
                            deduct_credits(uid, 1, session.blast_data)
                            batches_completed += 1
                            # Update current credits for display
                            current_credits = get_user_credits(uid, session.blast_data)
                    else:
                        # Admin/Owner: no credit deduction, but increment batches_completed for display
                        if batches_completed < display_count:
                            batches_completed += 1

                # ---- Cooldown trigger if needed ----
                if use_cooldown and batches_completed > 0 and (batches_completed % max(1, (count // 10))) == 0:
                    # After every ~10% of total batches, go to cooldown
                    cooldown_active = True
                    cooldown_until = time.time() + cooldown_interval
                    # Show cooldown in next iteration

                # Update progress every interval or on batch completion
                now = time.time()
                if (now - last_update_time >= _PROGRESS_UPDATE_INTERVAL) or (batch_sent > 0) or session.cancelled:
                    current_display_sent = batches_completed
                    current_display_fail = sent_fail // SMS_MULTIPLIER  # approximate
                    try:
                        await progress_msg.edit_text(
                            progress_text(current_display_sent, current_display_fail, display_count,
                                          current_credits if is_regular_user else None,
                                          speed_label_display,
                                          cooldown_left=0 if not cooldown_active else int(cooldown_until - time.time())),
                            reply_markup=stop_send_kb() if not session.cancelled else None,
                            parse_mode="HTML"
                        )
                    except TelegramBadRequest:
    ...
    if is_regular_user:
        # For regular users, we already deducted credits per batch; update stats
        ...
        if k in d_final["users"]:
            d_final["users"][k]["uses"] = d_final["users"][k].get("uses", 0) + batches_completed  # each batch counts as 1 use
        # Log history
        d_final.setdefault("sms_history", {}).setdefault(str(uid), []).append({
            ...
            "requested": display_count,
            "actual_sent": sent_ok,
            "actual_failed": sent_fail
        })
        # Update api usage
        ...
        # Admin/Owner: no credit deduction, update stats
        d_final["stats"]["total_sent"] = d_final["stats"].get("total_sent", 0) + sent_ok
        d_final["stats"]["total_failed"] = d_final["stats"].get("total_failed", 0) + sent_fail
        for fb_id, delta in api_usage_delta.items():
            d_final["stats"].setdefault("api_usage", {}).setdefault(fb_id, {"sent": 0, "failed": 0})
        ...
        k = str(uid)
        if k in d_final["users"]:
            d_final["users"][k]["uses"] = d_final["users"][k].get("uses", 0) + batches_completed
        d_final.setdefault("sms_history", {}).setdefault(str(uid), []).append({
            "number": number,
            "message": message[:100],
            "timestamp": int(time.time()),
    ...
    try:
        ...
        status_text = 'STOPPED BY USER' if was_cancelled else 'COMPLETED'
    chan_log = (
        f"<b>SMS BLAST ACTIVITY LOG</b>\n\n"
        f"<b>User:</b> {html_escape(u_name)}\n"
        f"<b>User ID:</b> <code>{uid}</code>\n"
        f"<b>Username:</b> @{html_escape(u_username)}\n"
    ...
    if is_regular_user:
        ...
        credit_text = f"\n{em(EMOJI_MONEY, '💰')} Credits Used: <b>{batches_completed}</b>\n{em(EMOJI_MONEY, '💳')} Remaining: <b>{remaining}</b>"
    ...
    try:
        await progress_msg.edit_text(
            ...
            f"{em(EMOJI_CHECK, '✅')} Sent (actual): <b>{sent_ok}</b> (×{SMS_MULTIPLIER})\n"
...
async def owner_deduct_all_credits_confirm(msg: Message, state: FSMContext):
    ...
    confirm_kb = InlineKeyboardMarkup(inline_keyboard=[
        [btn("✅ YES, DEDUCT", "owner:deduct_all_yes", EMOJI_CHECK, "✅", style="success"),
         btn("❌ NO, CANCEL", "owner:home", EMOJI_CROSS, "❌", style="danger")]
    ])
    await msg.answer(
...
async def owner_fj_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        ...
        [btn(
            "enable" if not fj.get("enabled") else "disable",
            "owner:fj:toggle",
            EMOJI_CHECK if not fj.get("enabled") else EMOJI_CROSS,
...
# ======================== REDEEM CODES (with unlimited key) ========================
...
async def owner_redeem_menu(cq: CallbackQuery, state: FSMContext):
    ...
    for code, data in list(codes.items())[:10]:
        if data.get("unlimited", False):
            status = f"{em(EMOJI_INFINITY, '♾️')} Unlimited"
        else:
            status = f"{em(EMOJI_CHECK, '✅')} Active" if data.get("uses_left", 0) > 0 else f"{em(EMOJI_CROSS, '❌')} Expired"
        text += f"<code>{code}</code> — {em(EMOJI_MONEY, '💰')}{data['credits']} — {status}\n"
    rows = [
        [btn("ɢᴇɴᴇʀᴀᴛᴇ ᴄᴏᴅᴇ", "owner:redeem:gen", EMOJI_CHECK, "➕", style="success")],
        [btn("ɢᴇɴᴇʀᴀᴛᴇ ᴜɴʟɪᴍɪᴛᴇᴅ ᴋᴇʏ", "owner:redeem:gen_unlimited", EMOJI_INFINITY, "♾️", style="success")],
...
async def owner_redeem_uses(msg: Message, state: FSMContext):
    ...
    # New format: ROHIT-MXD-<6 random uppercase letters>
    prefix = "ROHIT-MXD-"
    while True:
        code = prefix + "".join(random.choices(string.ascii_uppercase, k=6))
...
@R.callback_query(F.data == "owner:redeem:gen_unlimited")
async def owner_redeem_gen_unlimited(cq: CallbackQuery, state: FSMContext):
    d = load()
    if not is_owner(cq.from_user.id, d):
        await cq.answer("🚫 Owner Only!", show_alert=True)
        return
    ...
    while True:
        ...
        if code not in d.get("redeem_codes", {}):
            break
    d.setdefault("redeem_codes", {})[code] = {
        "credits": 999999999,  # huge value to indicate unlimited
        "uses_left": 1,        # one-time use
    ...
    save(d)
    await cq.answer("♾️ Unlimited Key Generated!", show_alert=True)
    await cq.message.edit_text(
        f"{em(EMOJI_INFINITY, '♾️')} <b>Unlimited Key Generated!</b>\n\n"
        f"Code: <code>{code}</code>\n"
        f"This key gives <b>UNLIMITED</b> credits to the user.\n"
        f"<i>It can be used only once.</i>",
        reply_markup=kb([(f"{sc('back')}", "owner:redeem:menu")]),
        parse_mode="HTML"
    )
...
async def user_redeem_done(msg: Message, state: FSMContext):
    ...
    if code_data.get("uses_left", 0) <= 0 and not code_data.get("unlimited", False):
    ...
    if uid in code_data.get("used_by", []):
        ...
        return
    # Apply credits
    if code_data.get("unlimited", False):
        # Set credits to a very high number (e.g., 1000000) but we'll treat as unlimited
        credits = 99999999
        add_credits(uid, credits, d, is_manual=False)
        msg_text = f"{em(EMOJI_INFINITY, '♾️')} <b>UNLIMITED CREDITS ACTIVATED!</b>\n\nYou now have <b>Unlimited</b> credits!"
    else:
        credits = code_data["credits"]
        add_credits(uid, credits, d, is_manual=False)
        msg_text = f"{em(EMOJI_GIFT, '🎉')} <b>Redeem Successful!</b>\n\n{em(EMOJI_MONEY, '💰')} +{credits} credits added!"
    # Update code usage
    if not code_data.get("unlimited", False):
...
async def user_stats(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        f"{em(EMOJI_CHECK, '📤')} SMS Sent (batches): <b>{udata.get('uses', 0)}</b>\n"
...
async def user_sms_history(cq: CallbackQuery, state: FSMContext):
    ...
    if not history:
        ...
        for i, entry in enumerate(reversed(history), 1):
            ...
            sent_actual = entry.get("actual_sent", "?")
            lines.append(f"{i}. [{ts}] {status_icon} <code>{mask_number(num)}</code> — {msg_preview}... (Req:{req}, Sent:{sent_actual})")
        text = "\n".join(lines)
...
async def user_info(cq: CallbackQuery, state: FSMContext):
    await cq.message.edit_text(
        ...
        f"<b>Features:</b>\n"
        f"• {em(EMOJI_ROCKET, '🚀')} 7× SMS multiplier (invisible)\n"
        f"• {em(EMOJI_WARNING, '⏱️')} Smart cooldown for large batches\n"
        f"• {em(EMOJI_STAR, '📊')} Advanced progress bar\n"
        f"• {em(EMOJI_GIFT, '🎁')} Referral & Redeem system\n\n"
...
async def owner_settings(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        [btn("sᴇᴛ ʀᴇғᴇʀʀᴀʟ ᴄʀᴇᴅɪᴛs", "owner:settings:ref", EMOJI_GIFT, "🎁", style="primary")],
        [btn("ʙᴀᴄᴋ", "owner:home", EMOJI_GEAR, "🔙", style="primary")]
    ]
    await cq.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode="HTML")
...
async def cmd_logs(msg: Message, state: FSMContext):
    ...
    if not is_owner(uid, d):
        ...
        return
    # Generate full log export as a file
    import io
    from aiogram.types import BufferedInputFile
    log_text = json.dumps(d, indent=2, ensure_ascii=False)
    file_bytes = io.BytesIO(log_text.encode("utf-8"))
    file_bytes.name = f"blast_bot_logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
async def _ka_ping(request):
    ...
    return aiohttp_web.Response(text=f"✅ Bot Online | Uptime: {uptime}s | Devices: {len(CACHED_DEVICES)}")
...
async def self_ping_loop():
    if not APP_URL:
        log.info("APP_URL not set — self-ping disabled")
        return
    await asyncio.sleep(60)
    while True:
        try:
            async with aiohttp.ClientSession() as s:
                async with s.get(f"{APP_URL}/ping", timeout=aiohttp.ClientTimeout(total=10)) as r:
                    log.info(f"Self-ping: {r.status}")
        except Exception as e:
            log.warning(f"Self-ping fail: {e}")
...
async def main():
    ...
    log.info("Scanner, backup & self-ping tasks started")
    try:
        await bot.send_message(
            MAIN_OWNER,
            f"{em(EMOJI_ROCKET, '🚀')} <b>SMS Blast Bot {_VERSION} Online!</b>\n@{me.username}\n"
            f"<code>{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</code>\n\n"
            f"{em(EMOJI_GEAR, '🔄')} <b>Background Scanner:</b> Active\n"
            ...
            f"{em(EMOJI_MONEY, '💰')} <b>7x Multiplier:</b> ENABLED\n"
            f"{em(EMOJI_WARNING, '⏱️')} <b>Cooldown System:</b> ACTIVE (>1000 req)\n"
            f"🌐 <b>Keep-Alive:</b> Port {KEEP_ALIVE_PORT} ACTIVE\n"
            f"🔁 <b>Self-Ping:</b> {'✅ ' + APP_URL if APP_URL else '⚠️ Set APP_URL env var'}\n"
