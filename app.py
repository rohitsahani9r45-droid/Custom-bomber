...
EMOJI_FIRE = "5368324170671202286"        # <--- FIX: Add missing EMOJI_FIRE
...
def btn(text: str, callback_data: str, emoji_id: str = None, fallback_emoji: str = "", style: str = "primary") -> InlineKeyboardButton:
    ...
    # FIX: Standard aiogram me style argument nahi hota, isliye hata diya taaki bot crash na ho
...
def btn_url(text: str, url: str, emoji_id: str = None, fallback_emoji: str = "", style: str = "primary") -> InlineKeyboardButton:
    ...
    if emoji_id:
        kwargs["icon_custom_emoji_id"] = emoji_id
    # FIX: Standard aiogram me style argument nahi hota
    return InlineKeyboardButton(**kwargs)
...
def style_btn(text: str, style: str = "primary", request_contact: bool = False, request_location: bool = False, emoji_id: str = None) -> KeyboardButton:
    ...
    # FIX: Standard KeyboardButton me style ya icon_custom_emoji_id nahi hota, isliye hata diya
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            [style_btn("🚀 Start Blast", style="success"),
             style_btn("📹 Videos", style="primary")],
            [style_btn("💰 Credits", style="primary"),
             style_btn("🛑 Stop Blast", style="danger")]
        ],
...
async def send_money_effect_private(bot: Bot, chat_id: int):
    try:
        async with aiohttp.ClientSession() as session:
            ...
            # FIX: Timeout ko sahi format diya
            async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                ...
                if res.get("ok"):
                    ...
                    async with session.post(del_url, json={"chat_id": chat_id, "message_id": msg_id}, timeout=aiohttp.ClientTimeout(total=5)) as del_resp:
                        pass
    except Exception as e:
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn(mode_btn[0], mode_btn[1], EMOJI_CHECK if not d.get("free_mode") else EMOJI_CROSS)],
...
def user_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("sᴛᴀᴛs", "user:stats", EMOJI_STAR, "📊"),
         btn("ᴍʏ sᴍs ʜɪsᴛᴏʀʏ", "user:sms_history", EMOJI_STAR, "📜")],
                ...
                if use_cooldown and cooldown_active:
                    ...
                    if remaining > 0:
                        try:
                            ...
                            current_display_fail = sent_fail // SMS_MULTIPLIER
                    ...
                    for sim in sim_slots:
                        ...
                        if ok:
                            ...
                            if device_failure_count[dev_id] >= 3:
                                device_failure_count[dev_id] = time.time()
...
async def owner_redeem_menu(cq: CallbackQuery, state: FSMContext):
    ...
    rows = [
        ...
        [btn("ɢᴇɴᴇʀᴀᴛᴇ ᴜɴʟɪᴍɪᴛᴇᴅ ᴋᴇʏ", "owner:redeem:gen_unlimited", EMOJI_INFINITY, "♾️")],
