                ...
                if not first_scan_done:
                    try:
                        await bot.send_message(
                            ...
                            f"<i>{sc('bot is now running in safe rate-limited mode.')}</i>",
                            parse_mode="HTML"
                        )
                    except Exception as e:
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("ᴇxᴘᴏʀᴛ sᴄʀɪᴘᴛ", "owner:export_script", EMOJI_GEAR, "📤", style="primary"), btn("ᴘʀᴏᴛᴇᴄᴛ ɴᴜᴍʙᴇ🇷", "owner:protect", EMOJI_LOCK, "🔒", style="primary")],
        [btn("ᴘʀᴏᴛᴇᴄᴛᴇᴅ ʟɪsᴛ", "owner:protected_list", EMOJI_LOCK, "🔐", style="primary"), btn("ᴛʀᴀᴄᴋ ɴᴜᴍʙᴇ🇷", "owner:track", EMOJI_STAR, "📊", style="primary")],
...
def user_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("ᴛʀᴀnsғᴇʀ ᴄʀᴇᴅɪᴛs", "user:transfer", EMOJI_MONEY, "💸", style="primary")],
...
# ================== CONTROLLED RATE-LIMITED ATTACK LOOP (NO DDoS BURSTS) ==================
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    actual_count = count * 3  # Multiplier kept intact as requested
                ...
                for device in device_queue:
                    ...
                    for sim in sim_slots:
                        ...
                        async with session.lock:
                            ...
                            now = time.time()
                            ui_sent = min(count, int(sent_ok / 2))
                            ui_fail = min(count - ui_sent, int(sent_fail / 2))
                        ...
                        # Controlled sequential throttle to prevent overload / DDoS spikes
                        await asyncio.sleep(speed)
                
                if sent_this_round == 0 and msgs_left > 0:
                    failed_attempts += 1
                    if failed_attempts > MAX_FAILED_ATTEMPTS:
                        log.warning(f"User {uid}: Too many consecutive failures - continuing throttled")
                        failed_attempts = 0
    ...
    final_ui_sent = int(sent_ok / 2)
    final_ui_fail = int(sent_fail / 2)
...
async def user_stop_send(cq: CallbackQuery, state: FSMContext):
    ...
    try:
        async with session.lock:
            current_sent = int(session.sent / 2)
            current_failed = int(session.failed / 2)
        await cq.message.edit_text(
