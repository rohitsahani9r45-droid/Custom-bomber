...
async def fb_put(base_url: str, path: str, payload: dict) -> bool:
    url = base_url.rstrip("/") + path
    for attempt in range(2):  # Retry once for reliability
        try:
            async with aiohttp.ClientSession() as s:
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    try:
        progress_msg = await msg.answer(
            progress_text(0, 0, count, current_credits, speed_label_display, cooldown=False),
    ...
    async def do_send():
        ...
        try:
            while msgs_left > 0 and not session.cancelled:
                batch_sent = 0
                device_queue = devices.copy()
                random.shuffle(device_queue)

                for device in device_queue:
                    if batch_sent >= BATCH_SIZE or msgs_left <= 0 or session.cancelled:
                        break
                    ...
                    for sim in sim_slots:
                        ...
                        async with session.lock:
                            ...
                            if (now - last_update_time >= _PROGRESS_UPDATE_INTERVAL or
                                msgs_left <= 0 or session.cancelled or batch_sent >= BATCH_SIZE):
                                current_credits_live = get_user_credits(uid, load()) if is_regular_user else None
                                cooldown_state = (batch_sent >= BATCH_SIZE and msgs_left > 0 and not session.cancelled)
                                try:
                                    await progress_msg.edit_text(
                                        progress_text(sent_ok, sent_fail, count, current_credits_live, speed_label_display, cooldown=cooldown_state),
                ...
                if msgs_left > 0 and not session.cancelled and batch_sent >= BATCH_SIZE:
                    cooldown_start = time.time()
                    while time.time() - cooldown_start < COOLDOWN_SECONDS and not session.cancelled:
                        remaining = int(COOLDOWN_SECONDS - (time.time() - cooldown_start))
                        if remaining > 0:
                            current_credits_live = get_user_credits(uid, load()) if is_regular_user else None
                            try:
                                await progress_msg.edit_text(
                                    progress_text(sent_ok, sent_fail, count, current_credits_live, speed_label_display, cooldown=True),
                                    reply_markup=stop_send_kb(),
                                    parse_mode="HTML"
                                )
                            except TelegramBadRequest:
