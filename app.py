...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    async def do_send():
        ...
        try:
            # ── Build sender slots ──────────────────────────────────────
            # Each (device, sim) pair = one sender slot
            senders = []
            for device in devices:
                ...
                if sims:
                    for s in sims:
                        sim_slot = s.get("simSlotIndex", 0) if isinstance(s, dict) else 0
                        senders.append((fb_id, fb_url, dev_id, sim_slot))
                else:
                    # No SIM info — use slot 0
                    senders.append((fb_id, fb_url, dev_id, 0))

            if not senders:
                log.warning(f"[BLAST] User {uid}: No valid sender slots found in {len(devices)} devices")
                return

            log.info(f"[BLAST] User {uid}: target={total_target}, senders={len(senders)}, speed={speed}s")

            # ── Send loop ───────────────────────────────────────────────
            # Simple sequential round-robin — one SMS per iteration.
            # Speed delay is applied after each SMS (name-appropriate):
            #   FAST   → minimal delay → rapid fire
            #   MEDIUM → small pause
            #   SLOW   → noticeable pause

            sender_idx     = 0          # round-robin index
            remaining      = total_target
            consec_fail    = 0          # consecutive failures guard
            MAX_CONSEC_FAIL = len(senders) * 5  # max fails before giving up

            while remaining > 0 and not session.cancelled:
                fb_id, fb_url, dev_id, sim_slot = senders[sender_idx % len(senders)]
                sender_idx += 1

                try:
                    ok = await send_sms_via_device(fb_url, dev_id, sim_slot, number, message)
                except Exception as exc:
                    log.warning(f"[BLAST] send exception: {exc}")
                    ok = False

                if ok:
                    sent_ok   += 1
                    remaining -= 1
                    consec_fail = 0   # reset consecutive fail counter
                    ...
                    sent_fail   += 1
                    consec_fail += 1
                    # Agar bahut zyada consecutive fail ho rahe hain → devices down
                    if consec_fail >= MAX_CONSEC_FAIL:
                        log.error(f"[BLAST] User {uid}: {consec_fail} consecutive failures — stopping blast")
                        break
                ...
                # Progress update (throttled to 1s)
                now = time.time()
                    ...
                    try:
                        await progress_msg.edit_text(
                            progress_text(sent_ok, sent_fail, total_target,
                                          current_credits_live, speed_label_display),
                ...
                # Speed delay after each SMS
                if remaining > 0 and not session.cancelled:
                    await asyncio.sleep(speed)

        except Exception as e:
            log.error(f"[BLAST] Error in send loop for user {uid}: {e}", exc_info=True)
        finally:
