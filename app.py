...
async def fb_put(base_url: str, path: str, payload: dict) -> bool:
    url = base_url.rstrip("/") + path
    for attempt in range(3):  # Retry 3 times for stability
        try:
            async with aiohttp.ClientSession() as s:
                # Timeout increased to 8 seconds to handle fast mode load
                async with s.put(url, json=payload, timeout=aiohttp.ClientTimeout(total=8)) as r:
                    if 200 <= r.status < 300:
                        return True
        except Exception as e:
