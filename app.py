...
_MAX_CONCURRENT_SENDS = 10  # Increased for more parallel sending

SPEED_FAST = 0.04      # ~12 SMS per second (was 0.2)
SPEED_MEDIUM = 0.2     # ~5 SMS per second (was 0.5)
SPEED_SLOW = 0.4       # ~2.5 SMS per second (was 1.0)
SPEED_DEFAULT = SPEED_MEDIUM

# Burst rest to avoid API throttling
BURST_SIZE = 150        # after this many sends
REST_TIME = 4.5         # seconds to rest
...
# ================== MODIFIED KB WITH COLOURED BUTTONS ==================
def _determine_style(text: str) -> str:
    """Return 'success', 'danger', or 'primary' based on button text."""
    low = text.lower()
    if any(k in low for k in ("cancel", "stop", "remove", "delete", "disable", "ban", "deduct", "reset", "danger")):
        return "danger"
    ...
    for row in rows:
        ...
        for item in row:
            if isinstance(item, tuple) and len(item) == 2:
                ...
                # If the text already contains an emoji, we keep it; else we add a fallback? Not needed.
                new_row.append(btn(text, callback, style=style))
            else:
                # If it's already a button, pass through
                new_row.append(item)
...
# ================== MODIFIED SEND LOOP WITH BURST REST AND FASTER SPEED ==================
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    sent_ok = 0
    sent_fail = 0
    msgs_left = count  # Exact count - no multiplication
    api_usage_delta = {}
    last_update_time = time.time()
    start_time = time.time()
    failed_attempts = 0
    MAX_FAILED_ATTEMPTS = 30  # Stop if all devices fail repeatedly

    # Burst tracking
    burst_counter = 0          # number of sends since last rest

    async def do_send():
        nonlocal sent_ok, sent_fail, msgs_left, last_update_time, failed_attempts, burst_counter

        # Use a semaphore to limit concurrency (send up to _MAX_CONCURRENT_SENDS at once)
        sem = asyncio.Semaphore(_MAX_CONCURRENT_SENDS)

        # We'll create a queue of send tasks, but we still need to respect the burst rest.
        # We'll keep the loop but send multiple messages concurrently using gather.
        try:
            while msgs_left > 0 and not session.cancelled:
                # Build a batch of tasks (up to _MAX_CONCURRENT_SENDS)
                tasks = []
                for _ in range(min(_MAX_CONCURRENT_SENDS, msgs_left)):
                    if session.cancelled:
                        break
                    # Pick a random device and SIM
                    ...
                    # Create a coroutine for this send
                ...
                for ok in results:
                    ...
                    async with session.lock:
                        if ok:
                            ...
                            # Track per‑firebase stats
                            fb_id = "unknown"  # we didn't track per device in this quick loop; we'll skip for simplicity
                            # But we can keep approximate: we'll count total sent/failed globally
                        else:
                            sent_fail += 1
                            failed_attempts += 1
                            burst_counter += 0  # failure doesn't count for burst rest? we'll still rest after total sent
                    ...
                    # Burst rest: after every BURST_SIZE successful sends, rest for REST_TIME seconds
                    if burst_counter >= BURST_SIZE and not session.cancelled:
                        await asyncio.sleep(REST_TIME)
                        burst_counter = 0

                    # Rate limiting: sleep for the specified speed (per send)
                    await asyncio.sleep(speed)

                # If no messages sent and still have messages left, check failures
    ...
    if not is_regular_user:
        ...
        # We skipped per‑fb tracking in this simplified concurrency, but we can add a global count.
        # For simplicity, we'll just track total.
        k = str(uid)
