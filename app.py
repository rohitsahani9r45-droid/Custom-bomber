...
def btn(text: str, callback_data: str, emoji_id: str = None, fallback_emoji: str = "", style: str = None) -> InlineKeyboardButton:
    """
    Create a colored inline button.
    style = "primary"  → Blue  (default)
    style = "success"  → Green
    style = "danger"   → Red
    """
    ...
    try:
        ...
        # Fallback: aiogram version style support nahi karta
...
def style_btn(text: str, style: str = "primary", request_contact: bool = False, request_location: bool = False, emoji_id: str = None) -> KeyboardButton:
    """Reply keyboard button with color style."""
    kwargs = {
        "text": sc(text),
...
# SLOW   = extra safe for poor connections
SPEED_FAST   = 0.3    # ~3 SMS/sec per sender slot (reduced from 0.05)
SPEED_MEDIUM = 0.7    # ~1-2 SMS/sec per sender slot (reduced from 0.3)
SPEED_SLOW   = 2.0    # 0.5 SMS per sec (reduced from 1.5)
...
async def fb_put(base_url: str, path: str, payload: dict) -> bool:
    """PUT data to Firebase. Retries up to 4 times with backoff."""
    url = base_url.rstrip("/") + path
    ...
    async with aiohttp.ClientSession(connector=connector) as s:
        for attempt in range(4):
            try:
                async with s.put(
                    url, json=payload,
                    timeout=aiohttp.ClientTimeout(total=15)  # Increased from 10 to 15 seconds
                ) as r:
                    if 200 <= r.status < 300:
                        log.info(f"fb_put SUCCESS attempt {attempt+1}: SMS sent")
                        return True
                    log.warning(f"fb_put HTTP {r.status} attempt {attempt+1}: {url}")
            except aiohttp.ClientError as e:
                log.warning(f"fb_put network error attempt {attempt+1}: {e}")
            except asyncio.TimeoutError:
                log.warning(f"fb_put timeout attempt {attempt+1}: retrying... {url}")
            except Exception as e:
                log.warning(f"fb_put unexpected error attempt {attempt+1}: {e}")
            if attempt < 3:
                wait_time = 0.5 * (attempt + 1)  # 0.5s, 1.0s, 1.5s waits
                log.info(f"fb_put retrying after {wait_time}s (attempt {attempt+1}/4)")
                await asyncio.sleep(wait_time)
    log.error(f"fb_put FAILED after 4 attempts: {url}")
    return False
...
async def send_sms_via_device(fb_url: str, dev_id: str, sim_slot: int, to: str, message: str) -> bool:
    """
    Send one SMS via a Firebase device.
    Each call uses a UNIQUE path (uuid) so multiple rapid sends
    never overwrite each other in the Firebase DB.
    """
    unique_key = uuid.uuid4().hex          # e.g. "a3f7c2..."  — guaranteed unique
    ...
    return await fb_put(fb_url, path, payload)
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    mode_btn_cb   = "owner:free:off" if d.get("free_mode") else "owner:free:on"
    mode_style    = "danger" if d.get("free_mode") else "success"
    return InlineKeyboardMarkup(inline_keyboard=[
        # Row 1 — Send + Firebase (Green + Blue)
        [btn("sᴇɴᴅ sᴍs",          "owner:send",          EMOJI_ROCKET, "📤", style="success"),
         btn("ᴍᴀɴᴀɢᴇ ғɪʀᴇʙᴀsᴇ",   "owner:fb:menu:0",     EMOJI_FIRE,   "🔥", style="primary")],
        # Row 2 — Videos + Super Admins (Blue + Blue)
        [btn("ᴍᴀɴᴀɢᴇ ᴠɪᴅᴇᴏs",     "owner:videos:menu",   EMOJI_VIDEO,  "📹", style="primary"),
         btn("ᴍᴀɴᴀɢᴇ sᴜᴘᴇʀ ᴀᴅᴍɪɴs","owner:owners:menu",  EMOJI_CROWN,  "👑", style="primary")],
        # Row 3 — Admins + Users (Blue + Blue)
        [btn("ᴍᴀɴᴀɢᴇ ᴀᴅᴍɪɴs",     "owner:admins:menu",   EMOJI_SHIELD, "🛡", style="primary"),
         btn("ᴠɪᴇᴡ ᴜsᴇʀs",         "owner:users:list",    EMOJI_STAR,   "👥", style="primary")],
        # Row 4 — Ban (Red) + Unban (Green)
        [btn("ʙᴀɴ ᴜsᴇʀ",           "owner:ban",           EMOJI_CROSS,  "🚫", style="danger"),
         btn("ᴜɴʙᴀɴ ᴜsᴇʀ",         "owner:unban:menu",    EMOJI_CHECK,  "✅", style="success")],
        # Row 5 — Broadcast + Stats (Blue + Blue)
        [btn("ʙʀᴏᴀᴅᴄᴀsᴛ",          "owner:broadcast",     EMOJI_BELL,   "📢", style="primary"),
         btn("ᴀᴘɪ sᴛᴀᴛs",          "owner:stats",         EMOJI_STAR,   "📊", style="primary")],
        # Row 6 — Activity + Pricing (Blue + Blue)
        [btn("ᴀᴄᴛɪᴠɪᴛɪ ʟᴏɢ",       "owner:activity",      EMOJI_GEAR,   "📜", style="primary"),
         btn("ᴘʀɪᴄɪɴɢ ᴘʟᴀɴs",      "owner:pricing:menu",  EMOJI_MONEY,  "💳", style="primary")],
        # Row 7 — Redeem (Green) + Add Credits (Green)
        [btn("ʀᴇᴅᴇᴇᴍ ᴄᴏᴅᴇs",       "owner:redeem:menu",   EMOJI_GIFT,   "🎁", style="success"),
         btn("ᴀᴅᴅ ᴄʀᴇᴅɪᴛs",        "owner:credits:add",   EMOJI_MONEY,  "💰", style="success")],
        # Row 8 — Deduct (Red) + Add All (Green)
        ...
        # Row 9 — Deduct All (Red) + Force Join (Blue)
        [btn("ᴅᴇᴅᴜᴄᴛ ᴀʟʟ",         "owner:deduct_all_credits",EMOJI_CROSS,"💰", style="danger"),
         btn("ғᴏʀᴄᴇ ᴊᴏɪɴ",         "owner:fj:menu",       EMOJI_BELL,   "🔗", style="primary")],
        # Row 10 — Settings + SMS History (Blue + Blue)
        [btn("sᴇᴛᴛɪɴɢs",           "owner:settings",      EMOJI_GEAR,   "⚙️", style="primary"),
         btn("sᴍs ʜɪsᴛᴏʀʏ",        "owner:sms_history",   EMOJI_STAR,   "📋", style="primary")],
        # Row 11 — Export + Protect Number (Blue + Blue)
        [btn("ᴇxᴘᴏʀᴛ sᴄʀɪᴘᴛ",      "owner:export_script", EMOJI_GEAR,   "📤", style="primary"),
         btn("ᴘʀᴏᴛᴇᴄᴛ ɴᴜᴍʙᴇʀ",     "owner:protect",       EMOJI_LOCK,   "🔒", style="primary")],
        # Row 12 — Protected List + Track (Blue + Blue)
        [btn("ᴘʀᴏᴛᴇᴄᴛᴇᴅ ʟɪsᴛ",     "owner:protected_list",EMOJI_LOCK,   "🔐", style="primary"),
         btn("ᴛʀᴀᴄᴋ ɴᴜᴍʙᴇʀ",       "owner:track",         EMOJI_STAR,   "📊", style="primary")],
        # Row 13 — Free Mode toggle (dynamic color)
        [btn(mode_btn_text, mode_btn_cb, EMOJI_CHECK if not d.get("free_mode") else EMOJI_CROSS, style=mode_style)],
        # Row 14 — Refresh (Blue)
        [btn("ʀᴇғʀᴇsʜ", "owner:refresh", EMOJI_GEAR, "🔄", style="primary")],
        # Row 15 — Contact Owner (URL)
        [btn_url("👑 ᴄᴏɴᴛᴀᴄᴛ ᴏᴡɴᴇʀ", "https://t.me/Rohit_mxd", style="success")],
    ])
...
def admin_kb(d: dict) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="👑 CONTACT OWNER", url="https://t.me/Rohit_mxd")],
    ])

def user_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        # Send SMS — Green (main action)
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    # User ne jitna maanga — utna hi bhejna hai, koi multiplier nahi
    total_target = count
    sent_ok      = 0
    sent_fail    = 0
    api_usage_delta = {}
    ...
    async def do_send():
        ...
        try:
            # ── Build sender slots ──────────────────────────────────────────
            # Each (fb_id, fb_url, dev_id, sim_slot) = one independent sender
            senders = []
            ...
            if not senders:
                log.error(f"[BLAST] User {uid}: 0 sender slots — check device list")
                return

            log.info(f"[BLAST] User {uid}: target={total_target}  senders={len(senders)}  speed={speed}s")

            # ── Concurrency semaphore ───────────────────────────────────────
            # Controls how many simultaneous Firebase PUTs run at once.
            # More senders = higher concurrency up to 30.
            ...
            async def blast_one(fb_id: str, fb_url: str, dev_id: str, slot: int) -> bool:
                async with sem:
                    try:
                        return await send_sms_via_device(fb_url, dev_id, slot, number, message)
                    except Exception as exc:
                        log.warning(f"[BLAST] blast_one exception: {exc}")
                        return False

            # ── Main loop ───────────────────────────────────────────────────
            # Strategy: fire a BATCH of tasks concurrently every iteration.
            # Batch size = number of senders (all devices fire at once).
            # After each batch, count successes → remaining -= successes.
            # Failed ones are retried in the next batch.
            # This guarantees every requested SMS eventually goes out
            # as long as at least one device is reachable.

            remaining   = total_target
            sender_idx  = 0          # round-robin across senders
            stall_count = 0          # how many consecutive batches had 0 success
            MAX_STALL   = 6          # give up after 6 full batches with 0 success

            while remaining > 0 and not session.cancelled:
                # Build one batch — size = min(senders, remaining)
                ...
                # Stall guard — if 0 success in a whole batch, devices may be down
                if batch_success == 0:
                    stall_count += 1
                    log.warning(f"[BLAST] User {uid}: batch stall {stall_count}/{MAX_STALL} — 0 success")
                    if stall_count >= MAX_STALL:
                        log.error(f"[BLAST] User {uid}: devices unresponsive — stopping after {sent_ok} sent")
                        break
                    # Back-off before retrying
                    await asyncio.sleep(1.0 * stall_count)
                else:
                    stall_count = 0   # reset stall counter on any success

                # Progress update (throttled)
                now = time.time()
                if now - last_update_time >= _PROGRESS_UPDATE_INTERVAL or remaining <= 0:
                    cred_live = get_user_credits(uid, load()) if is_regular_user else None
                    try:
                        await progress_msg.edit_text(
                            progress_text(sent_ok, sent_fail, total_target,
                                          cred_live, speed_label_display),
                ...
                # Speed delay between batches
                if remaining > 0 and not session.cancelled:
                    await asyncio.sleep(speed)

        except Exception as e:
            log.error(f"[BLAST] Fatal error for user {uid}: {e}", exc_info=True)
        finally:
    ...
    log_activity(d_log, "sms_blast", uid,
        f"Sent: {sent_ok}, Failed: {sent_fail}, Total: {total_target}, Duration: {fmt_duration(duration)}, Stopped: {was_cancelled}")
    save(d_log)
    ...
    chan_log = (
        ...
        f"📊 <b>Requested Count:</b> <b>{total_target}</b>\n"
...
# ===== KEEP-ALIVE SERVER (Render/Koyeb restart fix) =====
