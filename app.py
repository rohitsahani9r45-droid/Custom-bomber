...
SPEED_FAST = 0.5      # 2 SMS per second - Reliable
SPEED_MEDIUM = 1.0    # 1 SMS per second - Smooth & Safe
SPEED_SLOW = 2.0      # 0.5 SMS per second - Very Safe
...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    sent_ok = 0
    sent_fail = 0
    count = count * 3   # 3x — user 10 maange toh 30 jayega
    msgs_left = count
    api_usage_delta = {}
    ...
    async def do_send():
        ...
        try:
            # ✅ CONTINUOUS LOOP - jab tak msgs_left > 0 tab tak chale ga
            cycle = 0
            max_stall_count = 0  # Agar continuous fail ho to rukne ke liye
            MAX_STALL_ATTEMPTS = 10  # 10 continuous fails ke baad rukna

            while msgs_left > 0 and not session.cancelled:
                cycle += 1
                if cycle % 10 == 0:
                    log.info(f"User {uid}: cycle {cycle}, sent={sent_ok}, remaining={msgs_left}")
                ...
                for device in devices:
                    ...
                    async with session.lock:
                        if session.cancelled:
                            log.info(f"User {uid} stopped at cycle {cycle}, sent={sent_ok}")
                            break
                    ...
                    for sim in sim_slots:
                        ...
                        async with session.lock:
                            if ok:
                                sent_ok     += 1
                                batch_sent_in_cycle += 1
                                msgs_left   -= 1  # sirf success pe ghataao
                                max_stall_count = 0  # Reset stall counter on success
                                ...
                                sent_fail += 1
                                max_stall_count += 1
                                # msgs_left mat ghataao — agli cycle retry karega
                                if max_stall_count > MAX_STALL_ATTEMPTS:
                                    log.warning(f"User {uid}: Max stall reached - devices may be offline")
                                    break
                ...
                # Agar cycle me kuch bhi send nahi hua to wait karo phir retry karo
                if batch_sent_in_cycle == 0 and msgs_left > 0:
                    log.warning(f"User {uid}: No SMS sent in cycle {cycle}, retrying...")
