...
async def fb_put(base_url: str, path: str, payload: dict) -> bool:
    url = base_url.rstrip("/") + path
    for attempt in range(2):  # Sirf 2 baar try karega (Time bachega)
        try:
            async with aiohttp.ClientSession() as s:
                # Timeout 8 se kam karke 3 seconds kar diya gaya hai
                async with s.put(url, json=payload, timeout=aiohttp.ClientTimeout(total=3)) as r:
                    if 200 <= r.status < 300:
                        return True
        except Exception:
            pass  # Error log hata diya taki console hang na ho
        if attempt == 0:
