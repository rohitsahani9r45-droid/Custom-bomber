...
async def run_sms_blast_with_progress(bot: Bot, msg: Message, uid: int, number: str, message: str, count: int, devices: list, speed: float = SPEED_DEFAULT):
    ...
    sent_ok = 0
    sent_fail = 0
    # NO 3x multiplier — user ne jitna maanga utna hi bhejna hai
    msgs_left = count
    api_usage_delta = {}
    last_update_time = time.time()
    start_time = time.time()

    # Semaphore: ek saath max 50 concurrent firebase requests
    _BLAST_SEM = asyncio.Semaphore(50)

    async def send_one(fb_id: str, fb_url: str, dev_id: str, sim_slot: int) -> bool:
        """Single SMS send with semaphore control."""
        async with _BLAST_SEM:
            return await send_sms_via_device(fb_url, dev_id, sim_slot, number, message)

    async def do_send():
        nonlocal sent_ok, sent_fail, msgs_left, last_update_time

        try:
            # Build a flat task list: (fb_id, fb_url, dev_id, sim_slot) tuples
            # Each sim slot of each device counts as one sender
            senders = []
            for device in devices:
                ...
                for sim in sim_slots:
                    senders.append((fb_id, fb_url, dev_id, sim))

            if not senders:
                log.warning(f"No senders available for user {uid}")
                return

            log.info(f"[BLAST] User {uid}: {count} SMS requested, {len(senders)} sender slots available")

            # Round-robin: cycle through senders until msgs_left == 0
            sender_idx = 0
            BATCH_SIZE = min(50, max(10, len(senders)))  # batch = senders count ya 50, joh bhi kam ho

            while msgs_left > 0 and not session.cancelled:
                # Build a batch of tasks (up to BATCH_SIZE or msgs_left, whichever smaller)
                batch_count = min(BATCH_SIZE, msgs_left)
                batch_tasks = []
                batch_meta  = []  # (fb_id,) for each task

                for _ in range(batch_count):
                    if msgs_left <= 0 or session.cancelled:
                        break
                    fb_id, fb_url, dev_id, sim = senders[sender_idx % len(senders)]
                    batch_tasks.append(send_one(fb_id, fb_url, dev_id, sim))
                    batch_meta.append(fb_id)
                    sender_idx += 1
                ...
                async with session.lock:
                    for i, res in enumerate(results):
                        ...
                        if ok:
                            ...
                            sent_fail += 1
                            # Failed SMS ko retry karo — msgs_left mat ghataao
                    ...
                    # Progress update
                    now = time.time()
                    if now - last_update_time >= _PROGRESS_UPDATE_INTERVAL or msgs_left <= 0:
                ...
                if msgs_left > 0 and not session.cancelled:
