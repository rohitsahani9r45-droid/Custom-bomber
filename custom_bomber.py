...
BOT_TOKEN = "8995486066:AAHmXRG9VeDr47nEP61mq9g6InbNE5krpp8"
...
async def owner_add_all_credits_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        f"<i>Example: 10</i>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=sc("cancel"), callback_data="owner:home")]]),
        parse_mode="HTML"
    )
...
async def owner_add_all_credits_done(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(f"❌ {sc('Valid amount enter karein:')}", parse_mode="HTML")
        return
    await state.clear()
    
    count = 0
    for u_str in d.get("users", {}):
        add_credits(int(u_str), amount, d)
        count += 1
    save(d)
    
    await msg.answer(
        f"✅ <b>{sc('Success!')}</b>\n\n"
        f"Successfully added <b>{amount}</b> credits to <b>{count}</b> users.",
...
async def start_web_server():
    ...
    await site.start()
    log.info(f"Web server successfully started on port {port}")
    
async def main():
    await start_web_server()  # 👈 Yeh line yahan add karni hai 
